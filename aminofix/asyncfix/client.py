...
class Client(Callbacks, SocketHandler):
    def __init__(self, deviceId: str = None, socket_trace = False, socketDebugging = False, socket_enabled = True, autoDevice = False, sub: bool = False):
        self.api = "http://service.aminoapps.com/api/v1"
    ...
    async def login(self, email: str, password: str):
        ...
        **Returns**
        ...
        async with self.session.post(f"{self.api}/g/s/auth/login", headers=self.parse_headers(data=data), data=data) as response:
            ...
            if response.status != 200: return exceptions.CheckException(data)
            else:
                self.authenticated = True
