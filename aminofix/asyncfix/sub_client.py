...
class SubClient(client.Client):
    ...
    async def ban(self, userId: str, reason: str, banType: int = None, proxy: str = None):
        data = json.dumps({
            "reasonType": banType,
            "note": {
                "content": reason
        ...
        async with self.session.post(f"{self.api}/x{self.comId}/s/user-profile/{userId}/ban", headers=self.parse_headers(data=data), data=data, proxy=proxy) as response:
            if response.status != 200: return exceptions.CheckException(await response.text())
            else: return response.status
